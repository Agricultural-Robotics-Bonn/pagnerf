...
def _panoptic_quality_compute(
    ...
    dets_sum = true_positives+false_positives+false_negatives
    ...
    metrics = dict(
        all=dict(
            pq=torch.mean(panoptic_quality[dets_sum != 0]),
            rq=torch.mean(recognition_quality[dets_sum != 0]),
            sq=torch.mean(segmentation_quality[dets_sum != 0]),
            n=len(things) + len(stuff),
        ),
        things=dict(
            pq=torch.mean(panoptic_quality[: len(things)][dets_sum[: len(things)] != 0]),
            rq=torch.mean(recognition_quality[: len(things)][dets_sum[: len(things)] != 0]),
            sq=torch.mean(segmentation_quality[: len(things)][dets_sum[: len(things)] != 0]),
            n=len(things),
        ),
        stuff=dict(
            pq=torch.mean(panoptic_quality[len(things) :][dets_sum[len(things) :] != 0]),
