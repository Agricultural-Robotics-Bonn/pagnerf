#block(name=abl_pos_enc,threads=8,gpus=1,memory=10000,hours=6, subtasks=100)
  ...
  --log-dir "_results/logs/runs/best_models/best_pos_enc_mask2former_idrej_inst_sem_conf"\
  --exp-name "${frame_name}"\
  --epochs 200\
  --inst-epoch-start 0\
  --sem-epoch-start 0\
  ...
  --panoptic-features-type pos_encoding\
