#block(name=m2f_idrej,threads=8,gpus=1,memory=10000,hours=6, subtasks=100)
  ...
  --log-dir "_results/logs/runs/best_models/best_delta_mask2former_idrej"\
  --exp-name "${frame_name}"\
  --epochs 200\
  --inst-epoch-start 0\
  --sem-epoch-start 0\
  ...
  --load-modes imgs semantics instance preds_mask2former