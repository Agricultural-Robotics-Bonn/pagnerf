...
class NeRFStandard(DatasetFormatBase):
    ...
    def _parallel_load_standard_imgs(args):
        ...
        result = NeRFStandard._load_standard_imgs(args['frame'], args['root'], mip=args['mip'])
        if result is None:
    ...
    @staticmethod
    def load_data(root, split='train', bg_color='white', num_workers=-1, mip=None, *args, **kwargs):
        ...
        if num_workers > 0:
            ...
            p = Pool(num_workers)
            try:
                iterator = p.imap(NeRFStandard._parallel_load_standard_imgs,
            ...
            for frame in tqdm(transform_dict[split]['frames'], desc='loading data'):
                _data = NeRFStandard._load_standard_imgs(frame, root, mip=mip)
