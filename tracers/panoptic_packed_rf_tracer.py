...
class PanopticPackedRFTracer(PackedRFTracer):
    ...
    class).
    """
    def __init__(self, ray_sparcity_reg=0.0, ray_max_travel=6.0, **kwargs):
        super().__init__(**kwargs)
        self.render_channels = {'depth', 'alpha', 'hit'}
        self.base_channels = {'rgb', 'density'}
        self.panoptic_channels = {'semantics', 'inst_embedding'}
        
        self.ray_sparcity_reg = ray_sparcity_reg
        self.ray_max_travel = ray_max_travel
    ...
    def trace(self, nef, channels, extra_channels, rays,
        ...
        if raymarch_type == 'voxel' and depths.numel() != 0:
            # Filter out samples deeper than self.max_depth from the first hit for every ray
            _, counts_per_ray = ridx.unique(return_counts=True)
            # get the index of the first sample per ray
            ray_start_idx = torch.cumsum(counts_per_ray,dim=0)
            ray_start_idx = torch.cat([torch.tensor([0]).to(ray_start_idx.device),ray_start_idx[:-1]])
            # compute ray wise hit depth
            hit_depth = torch.take(depths[:,0,0], ray_start_idx)
            # compute traveled depth of each ray
            traveled_depth = depths[:,0,0] - torch.repeat_interleave(hit_depth, counts_per_ray)
            # Compute filter mask for samples within the specified max travel distance
            valid_mask = traveled_depth < self.ray_max_travel
            
            # Filter out deeper samples
            deltas = deltas.reshape(depths.shape)[valid_mask].reshape(-1,1)
            boundary = boundary.reshape(depths.shape)[valid_mask].reshape(-1)
            
            ridx = ridx[valid_mask]
            pidx = pidx[valid_mask]
        ...
        if any([c for c in channels if c in self.panoptic_channels]):
            panop_boundary = boundary.detach()
            panop_tau = tau.detach()
